...
/*   Updated: 2021/03/03 17:46:44 by mmonte           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_vert(t_set *s, int stripe, int start_y, int end_y)
{
	...
	y = start_y;
	s->tex->x = (int)(256 * (stripe - (-s->sp->width / 2 + s->sp->screen_x)) *
		s->tex->spr->wi / s->sp->width) / 256;
	if (s->sp->transform_y > 0 && stripe > 0 &&
		stripe < s->size_x && s->sp->transform_y < s->ray->buffer[stripe])
	{
		while (y < end_y)
		{
			d = y * 256 - s->size_y * 128 + s->sp->height * 128;
			s->tex->y = ((d * s->tex->spr->he) / s->sp->height) / 256;
			color = (*(int *)(s->tex->spr->addr + (s->tex->y *
				s->tex->spr->line_length + s->tex->x *
				(s->tex->spr->bits_per_pixel / 8))));
			if (color > 0)
...
void		draw_sprites(t_set *s)
{
	...
	while (i < s->sp->count)
	{
		...
		s->sp->coef = 1.0 / (s->pl.planey *
			s->pl.dirx - s->pl.diry * s->pl.planex);
		s->sp->transform_x = s->sp->coef *
			(s->pl.dirx * s->sp->x - s->pl.diry * s->sp->y);
		s->sp->transform_y = s->sp->coef *
			(-s->pl.planex * s->sp->x + s->pl.planey * s->sp->y);
		s->sp->screen_x = (int)((s->size_x / 2) *
